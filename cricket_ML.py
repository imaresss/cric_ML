...
types_of_delivery = ["over-pitched","leg-cutter","wrong'un","off-break","half-volley",'leg-side','length ball','length delivery','on pads','back of a length','short and wide','leg-break'
                     ,'in-ducking','full toss','googly','short in length','flatter delivery','outside off stump','full-toss',"on-side"
                 ...
                 'fuller delivery outside off', 'short' ,'bounce' ,'inswinger' ,'straighter delivery' ,'onto the stump',"onto the pads","on to the pads","off the pads" ,'flight delivery','full toss' ,
...
for i in range(29,10,-1):
    ...
    url = "https://m.cricbuzz.com/cricket-commentary/19165/rsa-vs-ind-6th-odi-india-tour-of-south-africa-2017-18/"+str(i)
    print(i)
    response = requests.get(url)
    html = response.text
    soup = BeautifulSoup(html,"html.parser")
...
##    the_text = str.lower(str(the_text))
    ...
    for each in reversed(the_text):
        ...
        if split[0].isdigit()  and (split[1]=="." or split[2]=="."):
            try:
                ...
                if any(c in text for c in ("short of a length","short of a good length", "back of a length","back of length","short of length")):
                ...
                elif  any(c in text for c in ("fuller", "full length","full","full ball")):
                    ...
                    bowling_area += check_after(bowling_area , text)
                elif any(c in text for c in ("quicker","quick delivery","quick ball","flatter", "flat length","flat ball",'flattish ball')):
    ...
    ##            print(runs)
                if '<b>' in runs:
                    ...
                    print(result)
                    conclude = check_four_six(result , split)
                    if conclude == -1:
                        out+=1
                    else:
                        ans += conclude
            ...
            try:
                ...
                df =df.append({'Batsman_Name': batsman ,'Over' : over , 'Bowler_Name': bowler , 'Batsman_country':"India" , 'Bowler_country':'South Africa',
                       'Score':ans , 'Run_rate':run_rate , 'Match_time': "Day" , 'Type_of_pitch':'Hard' , 'Bowling_area':bowling_area, 'Batting_area':batting_area ,
                               'Wickets':out , 'Result':conclude
                ...
                print("error")
save ="cricket_dataset_6th_odi.csv"
df.to_csv(save)
